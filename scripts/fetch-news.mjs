...
const ANGLES = [
  {
    ...
    words: ['省錢', '更省', '省油', '狂省', '划算', '養車', '保養費', '維修成本', '持有成本',
            '電費', '油錢', '補助', '補貼', '折抵', '換購', '優惠',
            '免費', '殘值', '保值', '入手價', '性價比', 'CP值', '零利率', '分期'],
  },
  {
    // 漲價與降價都放這裡：漲價能製造「再不買就變貴」的急迫感，對成交有幫助
    key: 'price', label: '價格異動',
    words: ['漲價', '調漲', '調高', '上調', '攀升', '看漲', '喊漲',
            '降價', '調降', '減價', '售價調整', '價格調整', '關稅', '零關稅'],
  },
  {
...
const OFFTOPIC = [
  ...
  // 傳聞中的低價車 —— 會讓客人「再等等」，一律不收
  '平價版', '便宜版', '廉價版', '平價', '更便宜', '便宜電動車', '低價車',
  // 供應鏈 / 半導體 / 週邊，與賣車無關
  //（'開箱' 刻意不列：會連 Model 3 / Y 的實車開箱評測一起濾掉）
  '台積電', '晶片', '三星', '英特爾', '抽獎', '行動電源', '配件',
];
...
const NEGATIVE = [
  ...
  '降至', '衰減', '減配', '砍配備',
  // 市場逆風（「其他」分類實測漏網補充）
  '逆風', '倒退', '讓位', '關鍵轉折', '失守', '節節', '邊緣化',
];
// 註：漲價／調高／攀升 刻意「不」列入否決 —— 漲價資訊能製造急迫感，對成交有幫助。
